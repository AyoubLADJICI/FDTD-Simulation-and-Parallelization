...
int main(int argc, char **argv)
{
  ...
  int rank, size;
  MPI_Comm_rank(MPI_COMM_WORLD, &rank); // Mon ID unique
  ...
  if(argc != 2) {
    ...
    MPI_Finalize(); // Finalisation de MPI avant de quitter
    return 1;
  }
  ...
  //Chaque processus alloue la grille entiere pour l'instant
  ...
  double start = GET_TIME(); // <-- Mauvaise mesure, on changera ça plus tard

  for(int n = 0; n < nt; n++) {
    if(rank == 0 && n && (n % (nt / 10)) == 0) { // Seul rank 0 affiche l'ETA
